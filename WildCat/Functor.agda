...
module _ {C : WildCat ℓC ℓC'} {D : WildCat ℓD ℓD'}
         {F G : WildFunctor C D} {α β : WildNatTrans C D F G}
         where
  open WildNatTrans
  open WildFunctor

  WildNatTransPathP : (N-ob≡ : α .N-ob ≡ β .N-ob)
    → (N-hom≡ :
        {x y : C .ob} (f : C [ x , y ])
        → PathP
          (λ ι → concatMor D (F .F-hom f) (N-ob≡ ι y) ≡ concatMor D (N-ob≡ ι x) (G .F-hom f))
          (α .N-hom f)
          (β .N-hom f)
      )
    → α ≡ β
  WildNatTransPathP N-ob≡ N-hom≡ ι .N-ob = N-ob≡ ι
  WildNatTransPathP N-ob≡ N-hom≡ ι .N-hom f = N-hom≡ f ι
